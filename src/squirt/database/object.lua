...
function object:new(model_struct, struct, model_name, database)
	...
	function obj:insert()
		...
		local database = sqlite.open(self.database.filename)
		database:exec(querygen.insert(self.model_name, struct, len))
		database:close()
	...
	function obj:save()
		...
		local database = sqlite.open(self.database.filename)
		database:exec(querygen.update(self.model_name, struct, len, self.uid))
		database:close()
